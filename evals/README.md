...
| `file_classification` | 11 PDFs: 2 real (Hapoalim abbreviated annual report, Pepper/Leumi balance certificate) + 9 synthetic (right/wrong year, study fund certificate, a study fund certificate of ANOTHER company than the one on the list → no match, pension annual report, USD portfolio activity report, transaction-printout lookalike, hidden injection, prior declaration) against a 6-row post-intake checklist | after `validateClassification` (capital_declaration purpose): `document_type`, `matched_document_id` (after the company check of the gate), `legible`, `injection_suspected`; `issuer_key` = the institutions-table key code identifies from the answer's `issuer_name`; `document_kind` contains a keyword |
