...
export async function scheduleDraftEmail(
  clientId: string,
  args: { subject: string; body: string; delayMs: number; reasoning?: string },
): Promise<{ emailId: string; jobId: string }> {
  const draft = await emails.insertDraft(clientId, args.subject, args.body, args.reasoning ?? null);
