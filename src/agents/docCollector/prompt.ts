...
export function buildTaxFetchSection(token: string, state: string, available: boolean, allowedActions: string[]): string {
  ...
  // (and its OTP email) may start; until then start_login is simply not offered.
  const canStart = allowedActions.includes('start_login');
  const startNowGuidance =
    'הלקוח אישר שהוא זמין. שלח `tax_fetch_action: "start_login"` יחד עם הודעת WhatsApp קצרה שמודיעה שאתה מתחיל עכשיו, שרשות המסים תשלח לו בעוד רגע קוד לתיבת האימייל שלו, ושברגע שהקוד מגיע יש לשלוח אותו אליך כאן ב-WhatsApp. המערכת תטפל בהזדהות ובקוד. אם הלקוח מסרב להשתמש ב-WhatsApp לשלב הזה — הסבר שבלי ערוץ מיידי אי אפשר להעביר את הקוד בזמן, הצע שישיג את הטופס בעצמו, ושלח `tax_fetch_action: "cancel"`. אם הוא מבקש לא להמשיך — `tax_fetch_action: "cancel"`.';
  const waitForClientGuidance =
    'הסברת ללקוח על שלב הקוד וביקשת שיאשר שהוא פנוי. מאז הוא עדיין לא ענה — המתן לתשובתו ואל תתחיל שום דבר בעצמך; ההזדהות (והקוד שיישלח לאימייל שלו) תתחיל רק אחרי אישור מפורש שלו, ואז תוכל לשלוח `start_login`. בינתיים המשך לשוחח רגיל (תזכורות על מסמכים אחרים וכו\'). אם הלקוח מבקש להפסיק — `tax_fetch_action: "cancel"`.';

  const stateGuidance: Record<string, string> = {
    none: 'ניתן להציע ללקוח למשוך עבורו את טופס ה-106 ישירות מאתר רשות המסים (יש לנו את פרטי ההזדהות שלו). אם יש לו כמה מעסיקים — נמשוך את הטפסים מכל המעסיקים באותו תהליך. כשמתאים בשיחה, הצע זאת ושלח `tax_fetch_action: "offer"` יחד עם ההודעה שבה אתה מציע. אפשר לציין כבר בהצעה שהתהליך כולל אימות קצר: רשות המסים שולחת קוד לאימייל של הלקוח, ואת הקוד נוח להעביר ב-WhatsApp.',
    offered:
      'הצעת ללקוח למשוך את הטופס. אם הלקוח הסכים — שלח `tax_fetch_action: "client_agreed"` יחד עם הודעה שמסבירה את הצעד הבא: רשות המסים תשלח לו קוד חד-פעמי לאימייל, ואת הקוד מעבירים לך ב-WhatsApp. אם השיחה מתנהלת כרגע באימייל, ההודעה הזו צריכה להציע לו לעבור ל-WhatsApp לשלב הזה, להסביר בקצרה למה (ראה עיקרון הערוץ למעלה) ולבקש את אישורו. אם הוא כבר מתכתב איתך ב-WhatsApp — פשוט שלח שם את ההסבר. אל תציג את התהליך כאילו כבר התחיל — ההזדהות מתחילה רק בצעד הבא. אם עדיין לא ברור אם הסכים — המשך לשוחח רגיל.',
    agreed: canStart ? startNowGuidance : waitForClientGuidance,
    wa_intro_sent: canStart ? startNowGuidance : waitForClientGuidance,
    awaiting_otp:
      'המערכת ממתינה שהלקוח יעביר ב-WhatsApp את הקוד שרשות המסים שלחה לאימייל שלו, ותטפל בו אוטומטית — פשוט המשך לשוחח רגיל. אל תבקש את הקוד שוב בעצמך. אם הלקוח אומר שלא קיבל קוד — הזכר לו לבדוק את תיבת האימייל (כולל ספאם), לא הודעות SMS. אם הלקוח שלח את הקוד בתשובה לאימייל — הסבר בעדינות שהקוד נקלט רק ב-WhatsApp ובקש שישלח אותו שם. אם הלקוח מבקש להפסיק — `tax_fetch_action: "cancel"`.',
    ...
    delivered: 'טפסי ה-106 כבר נמשכו ונשלחו ללקוח (טופס אחד מכל מעסיק). אין צורך להציע שוב.',
