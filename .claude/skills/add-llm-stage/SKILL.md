...
1. **Pure rules module** `<name>Rules.ts`: the zod schema of the model output and the gate
   `validate<Name>(raw, context)` returning `{ result: boolean, reason: string | null, ...checked }`.
...
2. **Tests** `tests/<name>Rules.test.ts` (`node:test`, `assert/strict`): one passing case, one per
   rejection reason, one boundary case. Add the file to the `test` script in `package.json`, run `npm test`.
3. **Runtime module**: `export const <NAME>_PROMPT` (template, placeholders `{{name}}`, fence token
   ...
   `detail.result` + `reason` + the fields the timeline prints; no raw ID numbers in `detail`).
