...
/**
 * Re-derives goal_status from the documents list after a manual change, like the
 * doc collector's version, with one delta: checking every box before the agent
 * finished interviewing is the accountant saying "the list is final" — the sticky
 * interview flag is stamped too, so a later replan cannot reopen the interview.
 * Reopening a document on a complete client reopens the goal and replans; the
 * interview flag stays, so the agent only chases the reopened document.
 */
async function onDocumentsChanged(clientId: string): Promise<void> {
  ...
  if (allCollected && client.goal_status === 'pending') {
    ...
    sendGoalCompleteEmail(client, docs).catch((err) =>
...
/** The annual-report assistant's determined-documents CRUD and due-date editing, composed into the workspace router. */
export function buildRouter(): Router {
  const router = Router();

  // Composed into the shared workspace router alongside other agent types'
  // routes — bail out to it when the active agent isn't this one.
  router.use((req, _res, next) => {
    if (req.agentInstance && req.agentInstance.agent_type !== 'annual_report_assistant') {
      next('router');
      return;
    }
    next();
