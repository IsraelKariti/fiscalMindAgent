...
function taxFetchPreamble(): string {
  return [
    ...
    'להציע את המשיכה ללקוח זה עניין של ניסוח בלבד — כתוב את ההצעה בגוף ההודעה, בלי לקבוע שום פעולה. פעולה נקבעת רק כשמשהו קורה בפועל:',
    '- `client_agreed` — הלקוח הסכים (על סמך השיחה); צרף הודעה שמסבירה את שלב הקוד. בהודעה הזו: תאר את הקוד בלשון עתיד — "כשנתחיל את המשיכה, יישלח אליך קוד" (הקוד נשלח בפועל רק עם `start_login`, בשלב הבא) — והסבר שכשהקוד יגיע, מעבירים אותו אלינו ב-WhatsApp. אם השיחה מתנהלת כרגע באימייל, זו בדיוק ההודעה שבה תציע ללקוח לעבור ל-WhatsApp לשלב הקוד (ראה "המשכיות בין הערוצים" למטה).',
