...
- [ ] 4.3 Run `npm run build:gui` and open the client in the monday custom object. Verify: the menu opens fully inside the iframe and actions work.
  - NOT TESTED at archive time (2026-09-18, archived with the user's approval). Done: `npm run build:gui` succeeded, `/monday-object` serves the bundle that contains the menu, and the menu was checked in a 336px-wide same-origin iframe (stays in view, flips upward). Not done: opening a menu inside monday itself.
