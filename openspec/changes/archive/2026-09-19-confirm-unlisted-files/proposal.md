...
- **Company-name check in code (owner decision 2026-09-19, strict form).** For document types that are always issued by a financial institution (bank balances, securities portfolios, pension / provident funds, study funds, life-insurance savings, mortgage balances) the file check also reports the company that issued the file. Code keeps a table of known Israeli banks, investment houses and insurers with their Hebrew and English name forms, finds the company of the file and the company named by the list item, and keeps an automatic match only when both are known and are the same company. A different company, or a company that code cannot identify on either side, cancels the match and the file stays unattached — so the agent asks the client.
- The same comparison guards the agent's own attaching of a file to an item: two known, different companies are always refused; when a company cannot be identified, the agent may attach the file only with the client's quoted words that the file belongs to that item.
...
- New pure module `institutions.ts` (company table + name lookup + comparison) with unit tests; `catalog.ts` — which types are institution-bound; `analyzeFileRules.ts` — `issuer_name` in the classifier answer and the `issuer_matches_item` check; `prompt.ts` — the file's analysis line shows the issuer and a cancelled match; `web` check labels (`i18n.tsx`) for the new check key.
