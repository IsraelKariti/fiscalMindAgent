...
export function buildTaxFetchSection(token: string, state: string, available: boolean, allowedActions: string[]): string {
  ...
  const stateGuidance: Record<string, string> = {
    none: 'ניתן להציע ללקוח למשוך עבורו את טופס ה-106 ישירות מאתר רשות המסים (יש לנו את פרטי ההזדהות שלו). כשמתאים בשיחה, הצע זאת ושלח `tax_fetch_action: "offer"` יחד עם ההודעה שבה אתה מציע. אפשר לציין כבר בהצעה שהתהליך כולל אימות קצר: רשות המסים שולחת קוד לאימייל של הלקוח, ואת הקוד נוח להעביר ב-WhatsApp.',
    offered:
